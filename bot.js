...
Hangz.onText(/\/start/, (msg) => {
  ...
  Hangz.sendPhoto(chatId, fs.readFileSync(logoPath), {
    caption: `Selamat datang di Bot Obfuscator JavaScript!\n\nPilih menu di bawah ini:`,
    reply_markup: {
      inline_keyboard: [
        [
          { text: 'Daftar Fitur', callback_data: 'feature_list' },
          { text: 'Obfuscate Hard', callback_data: 'obfuscate_hard' },
          { text: 'Bantuan', callback_data: 'help' },
        ],
        [{ text: 'Cek ID Telegram', callback_data: 'check_id' }],
        [{ text: 'Play Lagu', callback_data: 'play_list' }]
      ],
    },
  });
});
...
Hangz.on('callback_query', async (callbackQuery) => {
  ...
  if (data.startsWith('ytmp3|') || data.startsWith('ytmp4|')) {
    ...
    if (!link || !link.startsWith('https://')) {
      return Hangz.sendMessage(chatId, '❌ Link tidak valid.');
    }
    try {
      await Hangz.sendMessage(chatId, '⏳ Sedang memproses, mohon tunggu...');
      if (command === 'ytmp3') {
        const res = await ytdl.ytmp3(link);
        if (!res.status || !res.download?.url) {
          return Hangz.sendMessage(chatId, `❌ Terjadi kesalahan saat mengunduh audio.`);
        }
